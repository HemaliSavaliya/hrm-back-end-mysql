    ...
    try {
        ...
        if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
            ...
            upload(req, res, async function (err) {
                ...
                const { announcementTitle, announcementDetails, selectDepartment } = req.body;

                // Insert new announcement into the database
                const addAnnouncement = "INSERT INTO hrm_announcements (companyId, announcementTitle, announcementDetails, selectDepartment, document) VALUES (?, ?, ?, ?, ?)";

                connection.query(addAnnouncement, [req.user.companyId, announcementTitle, announcementDetails, selectDepartment, JSON.stringify(req.files.map(file => file.filename))], async (err, result) => {
                    if (err) {
                    ...
                    try {
                        ...
                        res.status(200).json({ success: true, message: "Announcement added successfully" });
                    } catch (error) {
    ...
    try {
        ...
        if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
            upload(req, res, async function (err) {
                ...
                connection.query("SELECT * FROM hrm_announcements WHERE id = ?", [annoId], async (err, results) => {
                    if (err) {
                    ...
                    if (req.files && req.files.length > 0) {
                        ...
                        try {
                            ...
                            console.error("Error uploading announcement files to FTP server: " + error);
                            return res.status(500).json({ error: "Internal Server Error" });
                        }
                    }
                    ...
                    const updateAnnouncementQuery = "UPDATE hrm_announcements SET announcementTitle = ?, announcementDetails = ?, selectDepartment = ?, document = ? WHERE id = ?";

                    connection.query(updateAnnouncementQuery, [announcementTitle, announcementDetails, selectDepartment, updatedDocumentList, annoId], (err, result) => {
                        if (err) {
...
module.exports.deleteAnnouncement = async (req, res) => {
    try {
        ...
        if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
            ...
            const deleteAnnouncementQuery = "UPDATE hrm_announcements SET deleted = true WHERE id = ?";

            connection.query(deleteAnnouncementQuery, [announcementId], async (err, result) => {
                if (err) {
                    console.error("Error deleting announcement", err);
                    return res.status(500).json({ error: "Internal Server Error" });
                }

                res.status(200).json({ message: "Announcement marked as deleted successfully" });
            });
        } else {
            // User is not an admin, deny access
            res.status(403).json({ error: 'Access denied' });
...
module.exports.deleteDocument = async (req, res) => {
    try {
        ...
        if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
            // Fetch the existing announcement document list from the database
            connection.query("SELECT document FROM hrm_announcements WHERE id = ?", [announcementId], async (err, results) => {
                if (err) {
                    console.error("Error fetching existing document list", err);
                    return res.status(500).json({ error: "Internal Server Error" });
                }
                ...
                if (results && results[0] && results[0].document) {
                    ...
                    try {
                        ...
                        // Update the announcement in the database
                        const updateAnnouncementQuery = "UPDATE hrm_announcements SET document = ? WHERE id = ?";
                        connection.query(updateAnnouncementQuery, [JSON.stringify(updatedDocuments), announcementId], (err, result) => {
                            if (err) {
                                console.error("Error updating announcement:", err);
                                return res.status(500).json({ error: "Internal Server Error" });
                            }
...
module.exports.announcementList = async (req, res) => {
    try {
        const sql = "SELECT * FROM hrm_announcements WHERE deleted = false AND companyId = ?";
