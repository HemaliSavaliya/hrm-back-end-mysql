...
module.exports.addAwards = async (req, res) => {
  try {
    ...
    if (req.user || (req.user.role === "Admin" && req.user.role === "HR")) {
      ...
      const userQuery =
        "SELECT id, name FROM hrm_employees WHERE id = ? AND name = ?";

      connection.query(
        userQuery,
        [req.body.employeeId, req.body.employeeName],
        async (userErr, userResults) => {
          ...
          const existingAwardQuery =
            "SELECT * FROM hrm_awards WHERE employeeId = ? AND companyId = ? AND employeeName = ? AND awardsName = ?";

          connection.query(
            existingAwardQuery,
            [
              userResults[0].id,
              userResults[0].companyId,
              userResults[0].name,
              req.body.awardsName,
            ],
            async (awardErr, awardResults) => {
              if (awardErr) {
              ...
              if (awardResults.length > 0) {
                ...
                return res
                  .status(400)
                  .json({ error: "Employee already has this award" });
              }

              // If the user exists and no existing award, proceed to save the award
              const insertQuery =
                "INSERT INTO hrm_awards (companyId, awardsName, awardsDetails, employeeId, employeeName, reward, deleted) VALUES (?, ?, ?, ?, ?, ?, ?)";
...
module.exports.updateAwards = async (req, res) => {
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      const { awardsName, awardsDetails, employeeId, employeeName, reward } =
        req.body;

      const awardId = req.params.id;

      // Update the award in the database
      const updateAwardQuery =
        "UPDATE hrm_awards SET awardsName = ?, awardsDetails = ?, employeeId = ?, employeeName = ?, reward = ? WHERE id = ?";

      connection.query(
        updateAwardQuery,
        [awardsName, awardsDetails, employeeId, employeeName, reward, awardId],
        (err, result) => {
          if (err) {
          ...
          res
            .status(200)
            .json({ success: true, message: "Awards updated successfully" });
        }
      );
    } else {
...
module.exports.deleteAwards = async (req, res) => {
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const deletedJobQuery =
        "UPDATE hrm_awards SET deleted = true WHERE id = ?";
...
module.exports.awardsList = async (req, res) => {
  try {
    const sql =
      "SELECT * FROM hrm_awards WHERE deleted = false AND companyId = ?";
