    ...
    try {
        ...
        if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
            ...
            connection.query(checkTodoQuery, [todoId], (err, result) => {
                ...
                const insertEventQuery = "INSERT INTO hrm_calendarevents (companyId, description, start, end, todoId) VALUES (?, ?, ?, ?, ?)";

                connection.query(insertEventQuery, [req.user.companyId, description, start, end, todoId], (err, result) => {
                    if (err) {
...
module.exports.updateCalendarEvent = async (req, res) => {
    try {
        ...
        if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
            ...
            const updatedEventsQuery = "UPDATE hrm_calendarevents SET description = ?, start = ?, end = ?, todoId = ? WHERE id = ?";

            connection.query(updatedEventsQuery, [description, start, end, todoId, eventId], (err, results) => {
                if (err) {
                    console.error("Error updating calendar event", err);
                    return res.status(500).json({ error: "Internal Server Error" });
                }

                res.status(200).json({ success: true, message: "Calendar Event updated successfully" });
            });
        } else {
            // User is not an admin and HR, deny access
            res.status(403).json({ error: "Access Denied" });
...
module.exports.deleteCalendarEvent = async (req, res) => {
    try {
        ...
        if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
            ...
            const deleteEventsQuery = "UPDATE hrm_calendarevents SET deleted = true WHERE id = ?";
...
module.exports.calendarEventList = async (req, res) => {
    try {
        const sql = "SELECT * FROM hrm_calendarevents WHERE deleted = false AND companyId = ?";
