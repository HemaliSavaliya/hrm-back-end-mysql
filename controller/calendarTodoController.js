...
module.exports.addCreateTodos = async (req, res) => {
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const addTodo =
        "INSERT INTO hrm_calendartodo (companyId, name, color, deleted) VALUES (?, ?, ?, ?)";

      connection.query(
        addTodo,
        [req.user.companyId, name, color, false],
        (err, result) => {
          if (err) {
          ...
          res.status(200).json({
            success: true,
            message: "Calendar Todo Added successfully",
          });
        }
      );
    } else {
  ...
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const updateTodoQuery =
        "UPDATE hrm_calendartodo SET name = ?, color = ? WHERE id = ?";
  ...
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const deleteTodoQuery =
        "UPDATE hrm_calendartodo SET deleted = true WHERE id= ?";
...
module.exports.createTodosList = async (req, res) => {
  try {
    const sql =
      "SELECT * FROM hrm_calendartodo WHERE deleted = false AND companyId = ?";
