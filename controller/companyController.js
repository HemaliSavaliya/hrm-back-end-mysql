...
async function downloadFileFromFTP(res, filename) {
    ...
    const remoteFilePath = `/domains/stackholic.io/public_html/HRM/upload/company_logo/${filename}`;
    ...
    try {
        ...
        return res.status(500).json({ error: "Error fetching company logo from FTP" });
    } finally {
    ...
    try {
        ...
        if (req.user && req.user.role === "SuperAdmin") {
            ...
            upload.single("companyLogo[]")(req, res, async function (err) {
                ...
                const { companyName, companyEmail, companyPan, companyGST, subscription, startDate, endDate } = req.body;

                // Insert the new company into the company table
                const insertSql = `INSERT INTO hrm_companys (companyName, companyEmail, companyPan, companyGST, subscription, startDate, endDate, companyLogo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;

                connection.query(insertSql, [companyName, companyEmail, companyPan, companyGST, subscription, startDate || null, endDate || null, req.file.filename || null], async (err, result) => {
                    if (err) {
                    ...
                    const remoteFilePath = `/domains/stackholic.io/public_html/HRM/upload/company_logo/${req.file.filename}`
        ...
        if (req.user && req.user.role === "SuperAdmin") {
            upload.single("companyLogo[]")(req, res, async function (err) {
                ...
                if (req.file) {
                    ...
                    updateCompanyQuery = "UPDATE hrm_companys SET companyName = ?, companyEmail = ?, companyPan = ?, companyGST = ?, subscription = ?, startDate = ?, endDate = ?, companyLogo = ? WHERE id = ?";

                    queryParams = [companyName, companyEmail, companyPan || null, companyGST || null, subscription, startDate, endDate, req.file.filename, companyId];
                } else {
                    // If no new logo is uploaded, update other details while keeping the existing logo
                    updateCompanyQuery = "UPDATE hrm_companys SET companyName = ?, companyEmail = ?, companyPan = ?, companyGST = ?, subscription = ?, startDate = ?, endDate = ? WHERE id = ?";

                    queryParams = [companyName, companyEmail, companyPan || null, companyGST || null, subscription, startDate, endDate, companyId]
                }

                connection.query(updateCompanyQuery, queryParams, async (err, result) => {
                    if (err) {
    ...
    try {
        ...
        if (req.user && req.user.role === "SuperAdmin") {
            ...
            connection.beginTransaction(err => {
                ...
                const checkCompanyQuery = "SELECT deleted FROM hrm_companys WHERE id = ?";
                ...
                connection.query(checkCompanyQuery, [companyId], (err, result) => {
                    ...
                    connection.query(deleteCompanyQuery, [companyId], (err, result) => {
                        ...
                        connection.commit(err => {
                            ...
                            const message = isDeleted ? "Company marked as undeleted" : "Company marked as deleted";
