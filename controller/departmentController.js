...
module.exports.addDepartment = async (req, res) => {
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const { departmentName, departmentHead, departmentEmail, status } =
        req.body;

      // Insert new department into the database
      const addDepartmentQuery =
        "INSERT INTO hrm_departments (companyId, departmentName, departmentHead, departmentEmail, startingDate, status, deleted) VALUES (?, ?, ?, ?, ?, ?, ?)";
  ...
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      department.departmentHead =
        req.body.departmentHead || department.departmentHead;
      department.departmentEmail =
        req.body.departmentEmail || department.departmentEmail;
      ...
      const updateDepartmentQuery =
        "UPDATE hrm_departments SET departmentHead = ?, departmentEmail = ?, status = ? WHERE id = ?";
  ...
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const updateStatusQuery =
        "UPDATE hrm_departments SET status = ? WHERE id = ?";
...
module.exports.departmentList = async (req, res) => {
  try {
    ...
    const getAllDepartmentsQuery =
      "SELECT * FROM hrm_departments WHERE companyId = ? AND deleted = false";
    ...
    if (teamMemberIds.length > 0) {
      ...
      const fetchNamesQuery =
        "SELECT id, name FROM hrm_employees WHERE id IN (?)";
      try {
        ...
        const departmentsWithTeamMembers = departments.map((department) => {
          const teamMembersData = JSON.parse(department.teamMembers);
          department.teamMembers = teamMembersData.map(
            (id) => teamMemberMap[id]
          );
          return department;
        });
