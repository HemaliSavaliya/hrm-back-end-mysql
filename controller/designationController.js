...
module.exports.addDesignation = async (req, res) => {
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const addDesignationQuery =
        "INSERT INTO hrm_designations (companyId, designationName, startingDate, status,deleted) VALUES (?, ?, ?, ?, ?)";

      connection.query(
        addDesignationQuery,
        [companyId, designationName, todayDate, status, false],
        (err, result) => {
          if (err) {
          ...
          res
            .status(200)
            .json({ success: true, message: "Designation added successfully" });
        }
      );
    } else {
  ...
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const updateStatusQuery =
        "UPDATE hrm_designations SET status = ? WHERE id = ?";
...
module.exports.designationList = async (req, res) => {
  try {
    ...
    const getAllDesignationsQuery =
      "SELECT * FROM hrm_designations WHERE companyId = ? AND deleted = false";
