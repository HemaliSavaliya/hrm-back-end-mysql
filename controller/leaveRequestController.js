...
module.exports.addLeaveRequest = async (req, res) => {
  try {
    ...
    if (req.user && (req.user.role === "HR" || req.user.role === "Employee")) {
      ...
      const leaveTypeDetailsQuery =
        "SELECT * FROM hrm_leavetypes WHERE leaveName = ?";

      connection.query(
        leaveTypeDetailsQuery,
        [req.body.leaveName],
        (err, results) => {
          if (err) {
          ...
          connection.query(
            previousMonthLeaveRequestsQuery,
            [req.user.id, previousMonthStartDate, previousMonthEndDate],
            async (err, previousMonthLeaveRequests) => {
              if (err) {
                console.error(
                  "Error fetching previous month leave requests",
                  err
                );
                return res.status(500).json({ error: "Internal Server Error" });
              }
              ...
              const previousMonthTotalBalanced =
                previousMonthLeaveRequests.reduce((total, request) => {
              ...
              const insertLeaveRequestQuery = `
                      ...
                      (companyId, userId, userName, userRole, leaveType, leaveName, applyingDate, totalDays, startDate, endDate, description, status, entitled, utilized, balanced, carriedForward, deleted) 
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    `;
...
module.exports.updateLeaveReqStatus = async (req, res) => {
  try {
    ...
    const findLeaveRequestQuery =
      "SELECT * FROM hrm_leaverequests WHERE id = ?";
    connection.query(
      findLeaveRequestQuery,
      [leaveRequestId],
      async (err, results) => {
        if (err) {
        ...
        const updateLeaveRequestQuery =
          "UPDATE hrm_leaverequests SET ? WHERE id = ?";
        connection.query(
          updateLeaveRequestQuery,
          [updateFields, leaveRequestId],
          async (err, result) => {
            if (err) {
...
module.exports.leaveRequestList = async (req, res) => {
  try {
    ...
    const fetchLeaveRequestsQuery =
      "SELECT * FROM hrm_leaverequests WHERE userId = ? AND companyId = ? AND deleted = false";
...
module.exports.leaveRequestListRoleWise = async (req, res) => {
  try {
    ...
    if (role === "Admin") {
      ...
      fetchLeaveRequestsQuery =
        "SELECT * FROM hrm_leaverequests WHERE deleted = false AND companyId = ?";
    } else if (role === "HR") {
      // If the user is HR, fetch only employee leave requests for the same company
      fetchLeaveRequestsQuery =
        "SELECT * FROM hrm_leaverequests WHERE userRole = 'Employee' AND companyId = ? AND deleted = false";
    } else {
...
module.exports.fetchLeaveBalance = async (req, res) => {
  try {
    ...
    const fetchLeaveTypesQuery =
      "SELECT leaveName, leaveBalance FROM hrm_leavetypes WHERE companyId = ? AND deleted = false";

    connection.query(
      fetchLeaveTypesQuery,
      [req.user.companyId],
      (err, leaveTypes) => {
        if (err) {
        ...
        connection.query(
          ...
          (err, leaveRequests) => {
            ...
            const leaveData = leaveTypes.map((leaveType) => {
              ...
              const leaveTypeRequests = leaveRequests.filter(
                (request) => request.leaveName === leaveName
              );

              const totalUtilized = leaveTypeRequests.reduce(
                (acc, curr) => acc + curr.utilized,
                0
              );
              const totalCarriedForward = leaveTypeRequests.reduce(
                (acc, curr) => acc + curr.carriedForward,
                0
              );
              const totalBalanced =
                leaveBalance - totalUtilized + totalCarriedForward;
