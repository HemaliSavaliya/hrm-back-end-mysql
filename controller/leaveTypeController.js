...
module.exports.addLeaveType = async (req, res) => {
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const addLeaveType =
        "INSERT INTO hrm_leavetypes (companyId, leaveName, leaveBalance, leaveStatus, leaveAddingDate, deleted) VALUES (?, ?, ?, ?, ?, ?)";

      connection.query(
        addLeaveType,
        [companyId, leaveName, leaveBalance, leaveStatus, todayDate, false],
        (err, result) => {
          if (err) {
          ...
          res
            .status(200)
            .json({ success: true, message: "Leave Type Added successfully" });
        }
      );
    } else {
...
module.exports.updateLeaveType = async (req, res) => {
  try {
    // Check if the user making the request is an admin
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      const { leaveName, leaveBalance, leaveStatus, leaveAddingDate } =
        req.body;

      const leaveTypeId = req.params.id;

      // Update the leave type in the database
      const updateLeaveTypeQuery =
        "UPDATE hrm_leavetypes SET leaveName = ?, leaveBalance = ?, leaveStatus = ?, leaveAddingDate = ? WHERE id = ?";

      connection.query(
        updateLeaveTypeQuery,
        [leaveName, leaveBalance, leaveStatus, leaveAddingDate, leaveTypeId],
        (err, result) => {
          if (err) {
...
module.exports.deleteLeaveType = async (req, res) => {
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const deleteLeaveTypeQuery =
        "UPDATE hrm_leavetypes SET deleted = true WHERE id = ?";
  ...
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const updateStatusQuery =
        "UPDATE hrm_leavetypes SET leaveStatus = ? WHERE id = ?";
...
module.exports.leaveTypeList = async (req, res) => {
  try {
    const sql =
      "SELECT * FROM hrm_leavetypes WHERE deleted = false AND companyId = ?";
