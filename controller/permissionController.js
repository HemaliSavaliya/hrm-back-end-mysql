...
module.exports.addPermission = async (req, res) => {
    try {
        if (req.user && req.user.role === "Admin") {
            ...
            const addPermissionQuery = "INSERT INTO hrm_permissions (companyId, option_name, role) VALUES (?, ?, ?)";

            connection.query(addPermissionQuery, [companyId, option_name, role], (err, result) => {
                if (err) {
...
module.exports.deletePermission = async (req, res) => {
    try {
        if (req.user && req.user.role === "Admin") {
            ...
            const checkPermissionQuery = "SELECT * FROM hrm_permissions WHERE companyId = ? AND option_name = ? AND role = ?";

            connection.query(checkPermissionQuery, [companyId, option_name, role], (err, results) => {
                if (err) {
                ...
                if (results.length > 0) {
                    ...
                    const updatePermissionQuery = "UPDATE hrm_permissions SET status = ? WHERE companyId = ? AND option_name = ? AND role = ?";

                    connection.query(updatePermissionQuery, [status, companyId, option_name, role], (updateErr, updateResult) => {
                        if (updateErr) {
                            console.error("Error updating permission status", updateErr);
                            return res.status(500).json({ error: "Internal Server Error" });
                        }

                        res.status(200).json({ success: true, message: "Permission status updated successfully" });
                    });
                } else {
                    // If no permission entry exists, insert a new permission with the provided status
                    const addPermissionQuery = "INSERT INTO hrm_permissions (companyId, option_name, role, status) VALUES (?, ?, ?, ?)";

                    connection.query(addPermissionQuery, [companyId, option_name, role, status], (addErr, addResult) => {
                        if (addErr) {
...
module.exports.getPermission = async (req, res) => {
    try {
        ...
        if (role === "Admin") {
            getPermissionQuery = "SELECT option_name FROM hrm_permissions WHERE companyId = ?";
        } else {
            getPermissionQuery = "SELECT option_name FROM hrm_permissions WHERE companyId = ? AND role = ? AND status = 'Yes'";
