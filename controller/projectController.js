    ...
    try {
        ...
        if (req.user && req.user.role === "Admin") {
            ...
            upload(req, res, async function (err) {
                ...
                const teamMembersArray = Array.isArray(req.body.teamMembers) ? req.body.teamMembers : [];
                const teamMembersJSON = JSON.stringify(teamMembersArray.map(member => member.name));
                const userIdJSON = JSON.stringify(teamMembersArray.map(member => parseInt(member.id)));

                // Insert new project into the database
                const addProjectQuery = "INSERT INTO hrm_projects (companyId, projectName, clientName, clientEmail, startDate, endDate, status, teamMembers, userId, document) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if (req.user && req.user.role === "Admin") {
            ...
            upload(req, res, async function (err) {
                ...
                if (Array.isArray(req.body.teamMembers)) {
                    project.teamMembers = req.body.teamMembers.map(member => member.name);
                }
                ...
                if (req.files && req.files.length > 0) {
                    ...
                    let existingDocuments = project.document ? JSON.parse(project.document) : [];
                    ...
                    try {
                        ...
                        console.error("Error uploading project files to FTP server:", error);
                        return res.status(500).json({ error: "Internal Server Error" });
                    }
                }

                // Save the updated project
                const updateProjectQuery = "UPDATE hrm_projects SET clientEmail = ?, endDate = ?, status = ?, userId = ?, teamMembers = ?, document = ? WHERE id = ?";

                connection.query(updateProjectQuery, [
                    project.clientEmail,
                    project.endDate,
                    project.status,
...
module.exports.updateStatus = async (req, res) => {
    try {
        ...
        if (req.user && req.user.role === "Admin") {
            ...
            const updateStatusQuery = "UPDATE hrm_projects SET status = ? WHERE id = ?";
...
module.exports.deleteProject = async (req, res) => {
    try {
        ...
        if (req.user && req.user.role === "Admin") {
            ...
            const deleteProjectQuery = "UPDATE hrm_projects SET deleted = true WHERE id = ?";
...
module.exports.deleteDocument = async (req, res) => {
    try {
        ...
        if (req.user && req.user.role === "Admin") {
            ...
            connection.query("SELECT document FROM hrm_projects WHERE id = ?", [projectId], async (err, results) => {
                if (err) {
                ...
                if (results && results[0] && results[0].document) {
                    ...
                    try {
                        ...
                        const updateAnnouncementQuery = "UPDATE hrm_projects SET document = ? WHERE id = ?";
                        connection.query(updateAnnouncementQuery, [JSON.stringify(updatedDocuments), projectId], (err, result) => {
                            if (err) {
                                console.error("Error updating project:", err);
                                return res.status(500).json({ error: "Internal Server Error" });
                            }
...
module.exports.projectsList = async (req, res) => {
    try {
        ...
        if (role === "Admin") {
            ...
            const getAllProjectsQuery = "SELECT * FROM hrm_projects WHERE deleted = false AND companyId = ?";

            connection.query(getAllProjectsQuery, [req.user.companyId], (err, result) => {
                if (err) {
            ...
            const getEmployeeProjectsQuery = "SELECT * FROM hrm_projects WHERE JSON_CONTAINS(userId, ?) AND status = ? AND deleted = false AND companyId = ?";

            connection.query(getEmployeeProjectsQuery, [`${id}`, "Active", req.user.companyId], (err, result) => {
                if (err) {
