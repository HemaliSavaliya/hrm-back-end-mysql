...
module.exports.addRole = async (req, res) => {
    try {
        ...
        if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
            ...
            const addRoleQuery = "INSERT INTO hrm_roles (companyId, roleName, date, status) VALUES (?, ?, ?, ?)";

            connection.query(addRoleQuery, [companyId, roleName, todayDate, status], (err, result) => {
                if (err) {
    ...
    try {
        ...
        if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
            ...
            connection.query(updateStatusQuery, [status, id], (err, result) => {
                ...
                res.status(200).json({ success: true, message: "Role status updated successfully" });
            });
        } else {
...
module.exports.getAllRolesName = async (req, res) => {
    try {
        const getAllRolesQuery = "SELECT roleName FROM hrm_roles WHERE companyId = ? AND status = 'Enable'";
