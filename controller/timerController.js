...
module.exports.addTimerData = (req, res) => {
    try {
        ...
        if (req.user && (req.user.role === "HR" || req.user.role === "Employee")) {
            ...
            const insertTimerQuery = `INSERT INTO hrm_timer_tracker (companyId, date, userId, userName, projectName, description, role, startTime, resumeTime, pauseTime, stopTime, hours, minutes, seconds, totalHours, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;

            const values = [req.user.companyId, startingDate, req.user.id, req.user.name, req.body.projectName || null, req.body.description, req.user.role, req.body.startTime, req.body.resumeTime, req.body.pauseTime, req.body.stopTime, req.body.hours, req.body.minutes, req.body.seconds, totalHours, status];
...
module.exports.timerList = async (req, res) => {
    try {
        ...
        const getTimerQuery = "SELECT * FROM hrm_timer_tracker WHERE deleted = false AND userId = ? AND companyId = ?";

        connection.query(getTimerQuery, [userId, req.user.companyId], (error, results) => {
            if (error) {
...
module.exports.timerListRoleWise = async (req, res) => {
    try {
        ...
        if (userRole === "Admin") {
            getTimerQuery = "SELECT * FROM hrm_timer_tracker WHERE companyId = ? AND deleted = false";
        } else if (userRole === "HR") {
            // HR can only see employee timer data
            getTimerQuery = "SELECT * FROM hrm_timer_tracker WHERE role = 'Employee' AND companyId = ? AND deleted = false";
        } else {
