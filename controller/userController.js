...
const uploadDocument = multer({ storage: storageDocument }).array("governmentDocument[]", 5);
...
async function uploadFileToFtp(files) {
  ...
  try {
    ...
    for (const file of files) {
      ...
      const remoteFilePath = `/domains/stackholic.io/public_html/HRM/upload/employee_document/${file.filename}`;
...
async function downloadFileFromFTP(res, filename) {
  const client = new ftp.Client();
  const remoteFilePath = `/domains/stackholic.io/public_html/HRM/upload/employee_document/${filename}`;
  ...
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      uploadDocument(req, res, async function (err) {
        ...
        const checkDepartmentQuery = "SELECT * FROM hrm_departments WHERE id = ?";

        connection.query(checkDepartmentQuery, [departmentId], async (checkDepartmentErr, departmentResult) => {
          ...
          connection.query(sql, params, async (err, result) => {
            ...
            try {
              ...
              const updateDepartmentQuery = "UPDATE hrm_departments SET teamMembers = JSON_ARRAY_APPEND(COALESCE(teamMembers, '[]'), '$', ?) WHERE id = ?";

              connection.query(updateDepartmentQuery, [result.insertId, departmentId], (updateErr, updateResult) => {
                if (updateErr) {
  ...
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      uploadDocument(req, res, async (err) => {
        ...
        const getCurrentDeptQuery = "SELECT department FROM hrm_employees WHERE id = ?";

        connection.query(getCurrentDeptQuery, [userId], (currentDeptErr, currentDeptResult) => {
          if (currentDeptErr) {
            console.error("Error fetching current department:", currentDeptErr);
            return res.status(500).json({ error: "Internal Server Error" });
          }
          ...
          connection.query(checkDeptQuery, [department], (checkDeptErr, deptResult) => {
            if (checkDeptErr) {
            ...
            connection.query(removeUserQuery, [userId, currentDepartmentId], (removeErr, removeResult) => {
              if (removeErr) {
                console.error("Error removing user from current department:", removeErr);
                return res.status(500).json({ error: "Internal Server Error" });
              }
              ...
              connection.query(addUserQuery, [parseInt(userId), department], (addErr, addResult) => {
                if (addErr) {
                  console.error("Error adding user to new department:", addErr);
                  return res.status(500).json({ error: "Internal Server Error" });
                }

                // Fetch the existing employee document list from the database
                const getDocumentListQuery = "SELECT governmentDocument FROM hrm_employees WHERE id = ?";
                connection.query(getDocumentListQuery, [userId], async (err, results) => {
                  if (err) {
                  ...
                  if (results && results[0] && results[0].governmentDocument) {
                    existingDocuments = JSON.parse(results[0].governmentDocument);
                  }
                  ...
                  if (req.files && req.files.length > 0) {
                    ...
                    try {
                      ...
                      console.error("Error uploading document files to FTP server: " + error);
                      return res.status(500).json({ error: "Internal Server Error" });
                    }
                  }
                  ...
                  connection.query(updateEmpQuery, [email, designation, department, address, mobileNo, alternateNumber, role, status, salary, bankAccountHolderName, bankAccountNumber, bankName, bankIFSCCode, bankBranchLocation, updatedDocumentList, userId], (updateEmpErr, updateEmpResult) => {
                    if (updateEmpErr) {
                      console.error("Error updating employee's department:", updateEmpErr);
                      return res.status(500).json({ error: "Internal Server Error" });
                    }
...
module.exports.deleteDocument = async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === "Admin") {
      // Fetch the existing employee document list from the database
      connection.query("SELECT governmentDocument FROM hrm_employees WHERE id = ?", [empId], async (err, results) => {
        if (err) {
          console.error("Error fetching existing document list", err);
          return res.status(500).json({ error: "Internal Server Error" });
        }
        ...
        if (results && results[0] && results[0].governmentDocument) {
          ...
          const updatedDocuments = existingDocuments.filter(document => !documentListToDelete.includes(document));

          // Delete files from FTP server
          try {
            // Update the announcement in the database
            const updateAnnouncementQuery = "UPDATE hrm_employees SET governmentDocument = ? WHERE id = ?";
            connection.query(updateAnnouncementQuery, [JSON.stringify(updatedDocuments), empId], (err, result) => {
              if (err) {
                console.error("Error updating employee:", err);
                return res.status(500).json({ error: "Internal Server Error" });
              }

              res.status(200).json({ success: true, message: "Specified documents deleted successfully" });
            });
          } catch (error) {
            console.error("Error deleting files from FTP server or locally:", error);
            return res.status(500).json({ error: "Internal Server Error" });
          }
        } else {
  ...
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      connection.query(updateEmployeeQuery, [userId], (err, result) => {
        ...
        connection.query(removeFromDepartmentQuery, [userId, userId], (removeFromDeptErr, removeFromDeptResult) => {
          if (removeFromDeptErr) {
            console.error("Error removing employee from department: ", removeFromDeptErr);
            return res.status(500).json({ error: "Internal Server Error" });
          }
...
module.exports.empList = async (req, res) => {
  try {
    const sql = "SELECT e.*, d.departmentName AS department FROM hrm_employees e LEFT JOIN hrm_departments d ON e.department = d.id WHERE e.deleted = false AND e.companyId = ?";
    ...
    connection.query(sql, [req.user.companyId], (err, results) => {
      ...
      if (results.length > 0) {
        ...
        results.forEach(result => {
          result.governmentDocument = JSON.parse(result.governmentDocument);
        });
...
module.exports.updatePassword = async (req, res) => {
  try {
    ...
    let employees = await query("SELECT * FROM hrm_employees WHERE email = ?", [email]);

    // Find the admin by ID
    let admins = await query("SELECT * FROM hrm_admins WHERE email = ?", [email]);
    ...
    if (bcrypt.compareSync(password, userToUpdate.password)) {
      ...
      if (employees.length > 0) {
        await query("UPDATE hrm_employees SET password = ? WHERE id = ?", [hashedPassword, userToUpdate.id]);
      }

      // Update password in admins table if the user is an admin
      if (admins.length > 0) {
        await query("UPDATE hrm_admins SET password = ? WHERE id = ?", [hashedPassword, userToUpdate.id]);
      }

      // Fetch the updated user document
      let updatedUser;
      if (employees.length > 0) {
        updatedUser = await query("SELECT * FROM hrm_employees WHERE id = ?", [userToUpdate.id]);
      } else if (admins.length > 0) {
        updatedUser = await query("SELECT * FROM hrm_admins WHERE id = ?", [userToUpdate.id]);
      }

      res.status(200).json({ message: "Password updated successfully", user: updatedUser[0] });
    } else {
...
module.exports.forgotPassword = async (req, res) => {
  try {
    ...
    if (req.user && (req.user.role === "Admin" || req.user.role === "HR")) {
      ...
      const updateEmpQuery = "UPDATE hrm_employees SET password = ? WHERE id = ?";

      connection.query(updateEmpQuery, [hashedPassword, id], async (updateEmpErr, updateEmpResult) => {
        if (updateEmpErr) {
          console.error("Error updating employee password: ", updateEmpErr);
          return res.status(500).json({ error: "Internal Server Error" });
        }
